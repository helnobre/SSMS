...
function ensureSaleClientReference(payload: SalePayload): SalePayload {
  if (payload.client_reference?.trim()) {
    return payload;
  }
...
function appendToOutbox(payload: SalePayload): void {
  ...
  queuedSales.unshift({
    ...
    status: "queued",
    attemptCount: 0,
    payload: ensureSaleClientReference(payload),
  });
...
export async function submitSale(payload: SalePayload): Promise<SaleSubmitResult> {
  const payloadWithReference = ensureSaleClientReference(payload);

  try {
    const response = await postSalePayload(payloadWithReference);
    return { status: "submitted", sale: response };
  } catch (error) {
    if (error instanceof ApiRequestError && error.isNetworkError) {
      appendToOutbox(payloadWithReference);
      return { status: "queued", sale: payloadWithReference };
    }

    return {
      status: "failed",
      sale: payloadWithReference,
...
export async function retryQueuedSales(queuedSaleIds?: string[]): Promise<{
  ...
  for (const queuedSale of queuedSales) {
    ...
    const payloadWithReference = ensureSaleClientReference(currentItem.payload);
    const syncingItem: QueuedSale = {
      ...currentItem,
      payload: payloadWithReference,
      status: "syncing",
